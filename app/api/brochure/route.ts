...
export async function GET(request) {
    ...
    try {
        ...
        // `error` is `unknown` in a catch under strict mode; narrow before
        // reading .message so a non-Error throw cannot itself throw here.
        const message = error instanceof Error ? error.message : String(error);
        return new NextResponse(`Error fetching brochure: ${message}`, {
            status: 500,
