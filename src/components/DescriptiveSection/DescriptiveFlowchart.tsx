...
const DescriptiveFlowchart = ({ titleStart, highlight, titleEnd, subHeading, steps }: ComponentProps) => {
    return (
                    ...
                    {steps.map((step, index) => (
                        ...
                        >
                            ...
                            <div className="relative mb-6">
                                <div className="w-20 h-20 rounded-2xl bg-white border border-gray-200 shadow-lg flex items-center justify-center text-3xl text-brand-blue group-hover:bg-brand-blue group-hover:text-white transition-colors duration-300">
                                    {step.icon ? (
                                ...
                                <div className="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-brand-blue text-white flex items-center justify-center text-sm font-bold shadow-md border-2 border-white">
                                    {index + 1}
                                </div>
                            </div>

                            {/* Equal Height Card */}
                            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-md hover:shadow-xl transition-all duration-300 w-full flex flex-col h-full">
                                <h3 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-brand-blue transition-colors duration-300">
                                    {step.title}
                                </h3>
                                <p className="text-gray-600 text-sm font-poppins leading-relaxed flex-grow">
                                    {step.description}
                                </p>
                            </div>
