...
const SectionHeader = ({ title, highlight, subtitle, centered = false }: ComponentProps) => (
...
const Avatar = ({ initials, bg, size = 'md' }: ComponentProps) => {
...
export const VideoGridCard = ({ video, index, onClick }: ComponentProps) => {
...
const VideoHeroCard = ({ hero, onClick }: ComponentProps) => {
  if (!hero) return null;
  return (
  <div
  ...
  >
    <div
      ...
      style={{
        ...
        // `backgroundGradient` is not a CSS property -- this declaration has
        // always been discarded by the browser, so removing it is a no-op
        // visually. Restore the intent deliberately (as `backgroundImage`)
        // rather than by accident, if the gradient is actually wanted.
...
export const VideoModal = ({ video, onClose }: ComponentProps) => {
  if (!video) return null;
  return (
...
export const VideoVault = ({ allStories, courses, settings }: ComponentProps) => {
  ...
  const [activeTab, setActiveTab] = useState<CmsData>(null);
...
const WrittenFeaturedCard = ({ story }: ComponentProps) => {
...
export const WrittenGridCard = ({ story }: ComponentProps) => {
  if (!story) return null;
  return (
...
export const WrittenStories = ({ allStories, courses, settings }: ComponentProps) => {
...
const VoicesCard = ({ data }: ComponentProps) => (
...
export const VoicesOfExcellence = ({ testimonials: propTestimonials }: ComponentProps = {}) => {
