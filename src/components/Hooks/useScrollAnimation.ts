 ...
 */
const useScrollAnimation = (options: CmsData = {}) => {
    const {
        threshold = 0.1,
        rootMargin = '0px',
    } = options;
    ...
    const elementRef = useRef<CmsData>(null);
    ...
    // `as const` keeps this a tuple. Without it TS widens to
    // (RefObject<any> | boolean)[], and the destructured ref is then typed
    // `RefObject | boolean`, which no ref prop accepts.
