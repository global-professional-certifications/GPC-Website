...
import MentorShowcase from '../About/MentorShowcase';
...
const TestimonialPrevArrow = ({ onClick }: ComponentProps) => (
  <button
    onClick={onClick}
...
const TestimonialNextArrow = ({ onClick }: ComponentProps) => (
  <button
    onClick={onClick}
...
export default function CiaEnrolmentLandingPage() {
  const [openFaqIndex, setOpenFaqIndex] = useState<CmsData>(null);

  const leadFormRef = useRef<CmsData>(null);
  const phoneInputRef = useRef<CmsData>(null);
  const itiRef = useRef<CmsData>(null);
