...
/**
 * `to` and `href` are alternatives, so both must be optional.
 *
 * Without this, TypeScript infers both as required from the destructuring and
 * every one of the 77 call sites that passes only `to` fails with "Property
 * 'href' is missing". They are also optional in the other sense: Card renders
 * <Link to={linkTo}> with linkTo undefined, which is the dead-link case handled
 * below.
 */
type LinkProps = Omit<ComponentProps<typeof NextLink>, 'href'> & {
    /** react-router's name for the destination. Alias of `href`. */
    to?: string;
    href?: string;
    children?: ReactNode;
};
