...
const Glossary = () => {
    const [activeLetter, setActiveLetter] = useState<CmsData>(null);
    ...
    const handleLetterClick = (letter) => {
        ...
        if (section) {
            // Was `//const stickyOffset = 32 * 20;` -- commented out while the
            // line below still referenced it, so this handler threw
            // "ReferenceError: stickyOffset is not defined" and the page never
            // scrolled. Pre-existing bug, not a migration regression.
            //
            // Sourced from LAYOUT_HEIGHTS rather than the abandoned 640px so
            // the target heading clears the sticky banner + navbar instead of
            // landing underneath them.
            const stickyOffset = LAYOUT_HEIGHTS.NOTIFICATION_BAR + LAYOUT_HEIGHTS.NAVBAR;
    ...
    const filteredGlossary = Object.entries(groupedGlossary).reduce((acc: CmsData, [letter, items]: [string, CmsData]) => {
