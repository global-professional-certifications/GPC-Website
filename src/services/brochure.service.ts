 ...
 */
export const getBrochureUrl = async (course: string) => {
 ...
 */
export const getBrochureSection = async (course: string) => {
  const key = String(course || '').toLowerCase().trim()
  if (!key) return null
 ...
 */
export const downloadBrochure = async (course: string, filename?: string) => {
