...
export const getPostsByTag = async (tag: string) => {
    // The GROQ parameter is $tagName, not $tag, deliberately.
    //
    // @sanity/client reserves `tag` in QueryParams and types it `never`:
    // it is the name of a fetch *request option* used for observability
    // (client.fetch(query, params, { tag: '...' })). Passing a GROQ parameter
    // called `tag` is flagged as "you're using a fetch option as a GROQ
    // parameter, this is likely a mistake".
    //
    // Here it genuinely was a GROQ parameter, so the filter worked -- but the
    // collision is a real hazard, not a typing nuisance: the two meanings are
    // one library decision apart from silently diverging, and the failure would
    // be a tag filter that quietly stops filtering.
    //
    // GROQ parameter names are arbitrary, so renaming costs nothing.
    const query = `*[_type == "post" && $tagName in tags && !(_id in path("drafts.**"))] | order(publishedAt desc) {
    _id,
    title,
    slug,
    description,
    ...
    return await client.fetch(query, { tagName: tag })
}
...
export const getRelatedPosts = async (postId: string, categoryIds: string[], limit = 3) => {
